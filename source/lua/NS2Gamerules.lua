...
Script.Load("lua/SurvivalGame.lua")
...
if Server then
    ...
    function NS2Gamerules:OnCreate()
        ...
        self.game = SurvivalGame( nil, nil )
    ...
    function NS2Gamerules:CanEntityDoDamageTo(attacker, target)
        ...
        Shared.Message( string.format("can: %s damage: %s", attacker:GetClassName(), target:GetClassName()))

        return  attacker:GetTeamNumber() ~= target:GetTeamNumber() ---CanEntityDoDamageTo(attacker, target, Shared.GetCheatsEnabled(), Shared.GetDevMode(), self:GetFriendlyFire())
    end
    ...
    function NS2Gamerules:OnEntityDestroy(entity)
        ...
        if self.game then
            self.game:OnEntityDestroy(entity)
        end
    ...
    function NS2Gamerules:ResetGame()
        ...
        self.game:Init(team2TechPoint, commandStructure1)
    ...
    function NS2Gamerules:GetTeams()
        return { self.team1, self.worldTeam, self.spectatorTeam }
    end
    ...
    function NS2Gamerules:CheckForConcedeSequence(autoConcededTeam)
        ...
        if autoConcededTeam then -- this team was forced to auto-concede
            ...
            local marineTeam = self:GetTeam(kMarineTeamType)
        ...
        DoConcedeSequence(concedingTeam)
    ...
    function NS2Gamerules:UpdateWarmUp()
        ...
        if gameState < kGameState.PreGame then
            ...
            local numPlayers = team1players - team1bots
    ...
    function NS2Gamerules:CheckGameStart()
        ...
        if self:GetGameState() <= kGameState.PreGame then
            ...
            if ((team1Commander) or Shared.GetCheatsEnabled()) and (not self.tournamentMode or self.teamsReady) then
    ...
    function NS2Gamerules:UpdateGame()
        if self.game then
            self.game:Update()
        end
    end
